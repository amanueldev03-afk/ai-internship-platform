...
## Phase 8 Implemented Features

### Task 8.1 — Save / Unsave Internships & Immediate State Synchronization

- **Backend Endpoints**:
  - `POST /api/internships/<id>/save/` — Save an internship for the authenticated student (prevents duplicate rows, handles 404 for invalid IDs, enforces student role authorization).
  - `DELETE /api/internships/<id>/save/` — Remove saved internship for the authenticated student (ensures user isolation, safe unsave idempotency).
  - `GET /api/internships/saved/` — Returns the authenticated student's saved internships with nested details.
